...
const getFullAnalytics = async (req, res) => {
    try {
        ...
        const currentAdQuery = await db_1.default.query(`
      SELECT 
    ...
    `);
        ...
        const lastMonthAdQuery = await db_1.default.query(`
      SELECT 
        COUNT(id) AS impressions,
        COUNT(DISTINCT device_id) AS accounts_reached,
        AVG(time_spent) AS atps
    ...
    `);
        ...
        const last7DaysAdQuery = await db_1.default.query(`
      SELECT 
    ...
    `);
        ...
        // 4. Video Analytics
        const videoCountResult = await db_1.default.query('SELECT COUNT(*) FROM "Video"');
        const totalVideos = parseInt(videoCountResult.rows[0].count, 10) || 0;
        const videoSumResult = await db_1.default.query('SELECT SUM(views) as views, SUM(likes) as likes, SUM(downloads) as downloads FROM "Video"');
